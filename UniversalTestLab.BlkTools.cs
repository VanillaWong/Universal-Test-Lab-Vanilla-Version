...
namespace UniversalTestLab
{
    internal static class BlkTools
    {
        ...
        public static string ConfigurePlayerFieldSupport(string text)
        {
            // 2026-09: user missions have no loadout screen, so the RB ground
            // consumables (repair kit / extinguisher / medkit) never get stock and
            // the action-bar buttons stay dead. Fix = two native mechanisms:
            //   1) a player-army capture_zone_circle (restoration zone) - same object
            //      the official tank PvE missions place for field repair
            //   2) a periodic unitRestore with modificationRestore, which tops up the
            //      consumable counts (official training missions use it to restore
            //      expendable modifications like the UCAV call)
            if (String.IsNullOrEmpty(text)) return text;

            // --- zone: read the player's current spawn position for the circle ---
            double px = 6.35, py = 41.5, pz = -622.3;
            Match youTm = Regex.Match(text, "name:t=\"You\"[^\\n]*\\n(?:[^\\n]*\\n){0,6}?tm:m=\\[\\[[^\\]]*\\][^\\]]*\\][^\\]]*\\]\\s*\\[([-0-9.eE+]+),\\s*([-0-9.eE+]+),\\s*([-0-9.eE+]+)\\]", RegexOptions.IgnoreCase);
            if (youTm.Success)
            {
                double x, y, z;
                if (double.TryParse(youTm.Groups[1].Value, NumberStyles.Float, CultureInfo.InvariantCulture, out x) &&
                    double.TryParse(youTm.Groups[2].Value, NumberStyles.Float, CultureInfo.InvariantCulture, out y) &&
                    double.TryParse(youTm.Groups[3].Value, NumberStyles.Float, CultureInfo.InvariantCulture, out z)) { px = x; py = y; pz = z; }
            }
            string zone =
                "\n  objectGroups{\n" +
                "    name:t=\"UTL Restore Zone\"\n" +
                "    tm:m=[[5, 0, 0] [0, 5, 0] [0, 0, 5] [" + px.ToString("0.###", CultureInfo.InvariantCulture) + ", " + py.ToString("0.###", CultureInfo.InvariantCulture) + ", " + pz.ToString("0.###", CultureInfo.InvariantCulture) + "]]\n" +
                ...
                "    }\n" +
                "  }\n";
            int uOpen = text.IndexOf("\nunits{", StringComparison.Ordinal);
            if (uOpen < 0) uOpen = text.IndexOf("units{", StringComparison.Ordinal);
            if (uOpen >= 0)
            {
                int uBrace = text.IndexOf('{', uOpen);
                if (uBrace >= 0) text = text.Insert(uBrace + 1, zone);
            }

            // --- consumable top-up trigger ---
            string kits =
                "\n  \"UTL Player Kits\"{\n" +
                "    is_enabled:b=yes\n" +
                "    comments:t=\"Tops up ground consumables (repair kit, extinguisher, medkit) every 10 s so the RB action-bar buttons work in a user mission.\"\n" +
                "\n" +
                "    props{\n" +
                ...
                "        ammoRestore:b=no\n" +
                "        fuelRestore:b=no\n" +
                "        modificationRestore:t=\"tank_tool_kit\"\n" +
                "        modificationRestore:t=\"manual_extinguisher\"\n" +
                "        modificationRestore:t=\"tank_medical_kit\"\n" +
                "        modificationRestore:t=\"tank_medical_kit_expendable\"\n" +
                "        target:t=\"You\"\n" +
                "      }\n" +
                "    }\n" +
                "\n" +
                "    else_actions{}\n" +
                "  }\n";
            int tOpen = text.IndexOf("\ntriggers{", StringComparison.Ordinal);
            if (tOpen < 0) tOpen = text.IndexOf("triggers{", StringComparison.Ordinal);
            if (tOpen >= 0)
            {
                int tBrace = text.IndexOf('{', tOpen);
                if (tBrace >= 0) text = text.Insert(tBrace + 1, kits);
            }
            return text;
        }
